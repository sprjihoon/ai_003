...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (tenantId, username, password) => {
    const response = await axios.post(`${API_BASE}/api/users/login`, { tenantId, username, password }, { withCredentials:true });
    const { token, user } = response.data;
    localStorage.setItem('token', token);
    localStorage.setItem('user', JSON.stringify(user));
    localStorage.setItem('tenantId', user.tenant_id);
    localStorage.setItem('tenantType', user.tenant_type || 'brand');
    setUser(user);
    return user;
  };
