...
router.put('/:id', auth, async (req, res) => {
  try {
    const inspection = await Inspection.findOne({ where:{ id:req.params.id, tenant_id:req.user.tenant_id }});
    if (!inspection) {
...
router.put('/details/:detailId', auth, async (req, res) => {
  try {
    const detail = await InspectionDetail.findOne({ where:{ id:req.params.detailId, tenant_id:req.user.tenant_id }});
    if (!detail) {
